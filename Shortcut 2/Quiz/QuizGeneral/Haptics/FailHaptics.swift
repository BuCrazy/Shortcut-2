...
struct FailHaptics {    
    ...
    func playCustomHapticPatter() {
        ...
        do {
            ...
            let hapticEvent2 = CHHapticEvent(eventType: .hapticTransient, parameters: [intensity, sharpness], relativeTime: 0.1)
            let hapticEvent3 = CHHapticEvent(eventType: .hapticTransient, parameters: [intensity, sharpness], relativeTime: 0.2)
            
            // Create a pattern with the events
            let pattern = try CHHapticPattern(events: [hapticEvent, hapticEvent2, hapticEvent3 ], parameters: [])
